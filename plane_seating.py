...
* Economy_plus passengers select their seats when they purchase their tickets.
...
get_avail_seats(plane,economy_sold):
    Parameters: 
    plane : a list of lists representing plaine
...
purchase_economy_plus(plane,economy_sold,name):
    ...
    This routine randomly selects a seat for a person purchasing economy_plus. Preference is given to window and front seats.
...
purchase_economy_block(plane,economy_sold,number,name):
    Purchase regular economy seats. As long as there are sufficient seats available, store the name and number of seats purchased in the economy_sold dictionary and return the new dictionary

fill_plane(plane):
  takes an empty plane and runs our simulation to sell seats and then seat the economy passengers. See comments in the function for details. 
...
def create_plane(rows,cols):
    """
    Returns a new plane of size rows x cols
